...
###########################################################################################################
    ...
    comp_num=df['company_account_number']
    try:
...
###################################################################################################################################
    ...
    if 0<=tcreationtime<=0.2:
        credit3=48;
        creditlimit=0.05*(Revenue);
    elif 0.2<tcreationtime<=0.4:
        credit3=57;
        creditlimit=0.055*(Revenue);
    elif 0.4<tcreationtime<=0.7:
        credit3=64;
        creditlimit=0.057*(Revenue);
    elif 0.7<tcreationtime<=1:
        credit3=71;
        creditlimit=0.059*(Revenue);
    elif 1<tcreationtime<=2:
        credit3=79;
        creditlimit=0.063*(Revenue);
    elif 2<tcreationtime<=5:
        credit3=86;
        creditlimit=0.12*(Revenue);
    elif 5<tcreationtime<=7:
        credit3=95;
        creditlimit=0.13*(Revenue);
    else: 
        credit3=103;
        creditlimit=0.15*(Revenue);
...
##############################################################################################
    ...
    record=[str(sync_id),str(company_number),str(Credit_Score),Creditlimit,Purchase_Score,str(Revenue),str(CreditRating),str(PurchaseRating)]#str(sync_id),acc_type,company_type,frennsid,year_number,    
    ...
    if len(updsql)==0:
        ...
        sql="UPDATE syncfinancialanalysis SET syncsupplier_id=%s,company_number=%s,creditscore=%s,creditlimit=%s,purchasescore=%s,revenue=%s,creditrating=%s,purchaserating=%s WHERE company_name='" + namez + "'  and frenns_id = '" + frenns + "'"   #,acc_type=%s,company_type=%s,frenns_id=%s,year_number=%s,                                      
...
def com_num(cn,frn):
    try:
        ...
        jf=pd.read_sql('select distinct name,frenns_id from syncinvoice where company_number="' + cn + '"', con=mysql)
        ff=jf['name']
        fu=np.array(ff)
        fr1=jf['frenns_id']
        frn1=np.array(fr1)
