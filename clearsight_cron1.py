# -*- coding: utf-8 -*-
"""
Created on Wed Jul  3 09:45:33 2019
@author: Yousuf
"""
    ...
    ###########################################################################################################
        ...
        ##################################################################################################################################################
            ...
            if ownersequity<=-10000000:
                ...
                credit0=20;
            elif 0<ownersequity<=10000000:
                credit0=((ownersequity/10000000)*105);
            ...
            if 0<debtr<=100:
                credit1=(142-((debtr/100)*142));
                purchase2=(220-((debtr/100)*220));
            elif debtr==0:
                credit1=35;
                purchase2=22*2;
            # elif 10<debtr<=25:
            ...
            if totalass<-3000000:
                ...
                credit2=20;
            elif 0<totalass<=50000000:
                credit2=((totalass/50500000)*56);
            ...
            if longdebt<0:
                ...
                credit3=25;
                purchase3=34;
            elif 0<longdebt<=20000000:
                credit3=(108-((longdebt/20000000)*108));
                purchase3=(121-((longdebt/20000000)*121));
            ...
            if currliab<=0:
                credit4=111;
            elif 0<currliab<=20000000:
                credit4=(108-((currliab/20000000)*108));
        ...
        #####################################################################################
            ...
            if 0<=incorporate_daz<=7:
                credit5=((incorporate_daz/7)*95);
                purchase7=((incorporate_daz/7)*95);
            ...
            if 0<due_date_days<=180:
                credit6=(86-((due_date_days/180)*86));
                purchase6=(76-((due_date_days/180)*76));
            ...
            if grossprofit==0:
                credit8=0;
                purchase8=0;
            elif 0<grossprofit<=95000:
                credit8=((grossprofit/95000)*65);
                purchase8=((grossprofit/95000)*65);
            ...
            # if ownersequity<0:
            #     credi0=45
            # credi0=((ownersequity/10000000)*120)
            # if debtr>100 and debtr<200:
            #     credit1=53
            # elif debtr>200:
            #     credit1=50
            # else:
            ...
            #     purchase2=44
            # else:
            #     purchase2=(220-((debtr/400)*220))

            # if totalass<0:
            #     credit2=29
            # else:
            #     credit2=((totalass/50500000)*60)
            # if longdebt<0:
            #     credit3=121
            # else:
            #     credit3=(110-((longdebt/20000000)*110))
            ...
            # else:
            #     credit4=(108-((currliab/20000000)*108))
            # if incorporate_days==100:
            #     credit5=12
            # else:
            #     credit5=((incorporate_days/8)*103)

            # if incorporate_days==100:
            #     purchase7=12
            # else:
            #     purchase7=((incorporate_days/8)*103)

            # if due_date_days<0:
            #     credit6=97
            # else:
            #     credit6=(97-((due_date_days/180)*97))
            ...
            #     purchase8=5
            # else:
            #     purchase8=((grossprofit/110000)*75)
        ...
        #################################################################################################################################
            ...
            #print(records)
            sql1 = "UPDATE status_of_update SET stopped_at=%s where id = '"+str(z)+"'"
            number=[number]
...
#years=str(year)
data=pd.read_sql('select distinct(CompanyNumber) from company_ann_reports', con=mysql)
reg_number=data['CompanyNumber']
#reg_number_without_duplicates=reg_number.drop_duplicates().reset_index(drop=True)
reg_number_array=np.array(reg_number)
#print("looping all the companies in the year: ",years)
def thread1():
    for i in tqdm(range(0,int(len(reg_number_array)/4))):
    #        regnum(reg_number_array[i])
        update=pd.read_sql("SELECT stopped_at FROM status_of_update WHERE id='"+str(1)+"'", con=mysql)
        update=update['stopped_at']
        update=np.array(update)
        
        if update[0]==reg_number_array[i]:
            continue
        else:
            regnum(reg_number_array[i],1)
def thread2():
    for i in tqdm(range(int(len(reg_number_array)/4),int(len(reg_number_array)/2))):
    #        regnum(reg_number_array[i])
        update=pd.read_sql("SELECT stopped_at FROM status_of_update WHERE id='"+str(2)+"'", con=mysql)
        update=update['stopped_at']
        update=np.array(update)
        ...
        if update[0]==reg_number_array[i]:
            ...
            regnum(reg_number_array[i],2)
def thread3():
    for i in tqdm(range(int(len(reg_number_array)/2),int(3*len(reg_number_array)/4))):
    #        regnum(reg_number_array[i])
        update=pd.read_sql("SELECT stopped_at FROM status_of_update WHERE id='"+str(3)+"'", con=mysql)
        update=update['stopped_at']
        update=np.array(update)
        ...
        if update[0]==reg_number_array[i]:
            ...
            regnum(reg_number_array[i],3)
def thread4():
    for i in tqdm(range(int(3*len(reg_number_array)/4),int(len(reg_number_array)))):
    #        regnum(reg_number_array[i])
        update=pd.read_sql("SELECT stopped_at FROM status_of_update WHERE id='"+str(4)+"'", con=mysql)
        update=update['stopped_at']
        update=np.array(update)
