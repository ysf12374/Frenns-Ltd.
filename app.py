...
def get_company_by_number(company_number):
    ...
    try:
        a_year_ago = datetime.datetime.now() - datetime.timedelta(days=1999)
        sql_query = " SELECT * " \
                    " FROM company_data" \
                    " WHERE `Accounts.LastMadeUpDate` >= {} and company_data.CompanyNumber = '{}' "\
                    " LIMIT {} order by `Accounts.LastMadeUpDate` desc ".format(a_year_ago.strftime('%d/%m/%Y'), company_number, config.DATA_FETCH_LIMIT)
        ...
        try:
            ...
            logger.debug(e)
        try:
            yresponse =  requests.get("https://api.companieshouse.gov.uk/company"+"/"+company_number+"/officers",  auth=(config.CLIENT_SECRET, ''))
