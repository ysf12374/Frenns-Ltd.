...
# host2='144.76.137.232'
# user2=''
# password2=''
# db2=''


# con2 = pymysql.connect(host=host2, user=user2, password=password2, db=db2, cursorclass=pymysql.cursors.
#                            DictCursor)
...
def document_fraud():
    ...
    if request.method == 'POST':
        try:
         ...
         update=cur.fetchall()
         if not update:
            sql="INSERT INTO invoice_fraud_pdf (file_name) values ('"+str(document_filename)+"') " 
            cur.execute(sql)
            mysql.connection.commit()
            cur.close()
         else:
...
@app.route('/fraud_result', methods=['GET']) 
def fraud_result():
    status={"result":False,"message":"not executed"} 
    if request.method == 'GET':
        try:
            ...
            document=document[2:-3]
            import pdf_image
            fraud=pdf_image.pdf(document,frn_id,name,address,city,post)
...
@app.route('/fraud_invoice', methods=['GET']) 
def fraud_invoice():
    status={"result":False,"message":"not executed"} 
    if request.method == 'GET':
        try:
...
@app.route('/invoice_quality', methods=['GET']) 
def invoice_quality():
    status={"result":False,"message":"not executed"} 
    if request.method == 'GET':
        try:
            ...
            fraud=quality_image.q_img(document)
            # cur.execute("SELECT frenns_id FROM frenns_app_api WHERE frenns_app_api.frenns_id='"+str(frn_id)+"'")
            # update=cur.fetchall()
            # if not update:
            #     sql="INSERT INTO frenns_app_api (frenns_id, firstname, lastname, gender, dob, passport_number) values ('"+str(frenns_id)+"','"+firstname+"','"+lastname+"','"+gender+"','"+str(dob)+"','"+passport_number+"') " 
...
if __name__ == '__main__':
	# app.run(host='127.0.0.1', port=3996)#
